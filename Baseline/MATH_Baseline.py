...
def main(llm_type, dataset, iter_flag, max_iter, huggingface_model, task_type):  
    ...
    baseline = GJ_Module(model, tokenizer, huggingface_model)

    final_result = []
    while iter_flag <= max_iter:
        if iter_flag < max_iter:
            print(f'=============== inference {iter_flag} start ===============') 
            dataset, iter_flag = baseline.Generator(dataset, iter_flag, llm_type, task_type)
            dataset, finish_dataset, iter_flag = baseline.Judge(dataset, iter_flag, llm_type, task_type)
...
if __name__ == "__main__":
    ...
    text, final_acc, wrong = MATH_evaluator(final_result)
