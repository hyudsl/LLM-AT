...
def simulation(dataset, tier_list: list, data_pool: list, top_k: int, th: float, lambda_: float, acc_list: list, vectorstore, task_name):
    ...
    for item in tqdm(dataset):
        ...
        saver['uid'] = uid
        saver['question'] = item['question']
        saver["meta"] = item["meta"]
        saver["correct index"] = item["correct index"]
        saver["Choices"] = item['Choices']
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="LLM-AT: MCQA simulation")
    ...
    for i, model in enumerate(reversed(tier_list), start=1):
        globals()[f'{model}'] = json.load(open(f'/home/injae/LLM-AT/dataset/MCQA/sample/LLMs/{model}_COT.json'))
    ...
    text, final_acc, wrong = MCQA_evaluator(simulation_result)
    total_cost, total_time = cost_evaluator(simulation_result)

    if save:
        save_path = base_path + f'MCQA_LLMAT_{threshold}_top-{top_k}_{lambda_}_S.json'    
